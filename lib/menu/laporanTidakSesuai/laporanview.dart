      ...
      body: Center(
        child: InkWell(
          ...
          child: Hero(
              tag: tag,
              child: Image.network(
                '${globals.globalurlphoto}' + photourl,
                errorBuilder: (context, Object exception, stackTrace) {
                  print('a');
                  return const Text('Your error widget2');
                },
              )),
        ),
      ),
...
class _laporanview extends State<laporanview> {
  ...
  Future<void> getFoto({String? search, String? batch, String? lot}) async {
    ...
    final Uri url = Uri.parse('${globals.globalurl}/getlaporanfoto?rcptnbr=' +
        search.toString() +
        '&batch=' +
        batch.toString() +
        '&lot=' +
    ...
    if (responseresult != []) {
      responseresult?.asMap().forEach((index, element) {
        var responsecheck = http
            .head(Uri.parse('${globals.globalurlphoto}' + element['li_path']))
            .then((responsecode) {
  ...
  void initState() {
    ...
    Komplain = TextEditingController(
        text: widget.komplain != 'null' ? widget.komplain : '-');
    Keterangan = TextEditingController(
        text: widget.keterangan != 'null' ? widget.keterangan : '-');
    ...
    createdby = TextEditingController(
        text: widget.createdby != 'null' ? widget.createdby : '');
  }
